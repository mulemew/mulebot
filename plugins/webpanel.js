 ...
 *   - it binds 127.0.0.1 unless the platform injected PORT, in which case it
 *     binds 0.0.0.0 because that is the only way to be reachable there — and on
 *     such a host the credential is the only thing in front of it
...
module.exports = {
  ...
  init(plugin) {
    ...
    // injects PORT is a host that will not keep the process alive without a
    // listener on it. Binding anywhere else there means the panel is
    // unreachable and the deployment still gets restarted.
    const port = Number(config.port || process.env.WEBPANEL_PORT || process.env.PORT || 8787);

    // 0.0.0.0 only when the port came from the platform, since a platform port
    // is reached from outside the container by definition. Everywhere else this
    // stays on localhost.
    //
    // Be clear about what that means: on a PaaS this panel is on the public URL
    // of the service, and the credential is the only thing in front of it. The
    // panel uploads and runs code inside the bot process, so that credential is
    // equivalent to a shell on the host. It is rate-limited, locks out after
    // repeated failures and stores only a scrypt verifier - but a long random
    // secret is doing the real work, so generate one rather than choosing one.
    const usingPlatformPort = !config.port && !process.env.WEBPANEL_PORT && Boolean(process.env.PORT);
    const host = config.host || (usingPlatformPort ? '0.0.0.0' : '127.0.0.1');
